...
export default function InventoryTab({ user }) {
  ...
  const handleEmailMoving = async () => {
    ...
    const list = movingItems.map(i => `• ${i.name} — ${SIZE_LABELS[i.size]}: ${i.sizeLabel} (${i.room})`).join("\n");
    const body = `Moving Inventory — Quote Request\n================================\n${list}\n\nEstimated Truck: ${truck?.label || "TBD"} (${truck?.truck || ""})\n\nGenerated by EZ Move AI`;
  ...
  const handleEmailDonate = async () => {
    ...
    const list = donateItems.map(i => `• ${i.name} — ${SIZE_LABELS[i.size]}: ${i.sizeLabel} (${i.room})`).join("\n");
