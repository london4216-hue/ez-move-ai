...
import { Sparkles, ChevronDown, ChevronUp, FileText, Package, Clock, MapPin, DollarSign, FileCheck, Save, Check, UtensilsCrossed } from "lucide-react";
...
function AIGenerator({ feature, user, onSave }) {
  ...
  const prompts = {
    ...
    delivery: `You are a logistics expert. Recommend the best delivery and moving service options for someone moving to ${user?.home_address || "their new location"} with a closing date of ${user?.estimated_close_date || "soon"}. Compare at least 3-4 options including: traditional moving companies, freight services, container services (PODS, U-Pack), and hybrid options. Include estimated costs, pros/cons, and customer reviews for each. Format as a comparison list.`,
