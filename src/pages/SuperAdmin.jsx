...
function Sidebar({ active, onSelect, onLogout, collapsed, onToggle }) {
  return (
    <aside className={`flex-shrink-0 bg-slate-900 flex flex-col transition-all duration-200 ${collapsed ? "w-16" : "w-56"}`}>
      {/* Logo */}
      ...
      <div className="px-2 py-4 border-t border-white/10 space-y-1">
        <button onClick={onToggle}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-500 hover:text-white hover:bg-white/10 text-sm font-semibold transition-all">
          <Menu className="w-4 h-4 flex-shrink-0" />
          {!collapsed && <span>Collapse</span>}
        </button>
        <button onClick={onLogout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-500 hover:text-red-400 hover:bg-red-400/10 text-sm font-semibold transition-all">
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        {/* Top bar */}
        <header className="bg-white border-b border-slate-200 shadow-sm px-6 py-4 flex items-center justify-between flex-shrink-0">
          <div>
